...
class UIManager extends Manager {
    ...
    updateHighscore() {
        ...
        if (t.timestamps.global == "undefined" || (serverTime.getTime() - t.timestamps.global > 3600000)) {
            const e = {
                0: "global",
                1: "economy",
                2: "research",
            ...
            0 == currentType && Util.runAsync((() => this.displayScoreDiff(t))), (!t.timestamps?.[e[currentType]] || serverTime.getTime() - t.timestamps?.[e[currentType]] > 3600000) && this.ogl._fetch.pending.push({
    ...
    updateStatus() {
        ...
        if (e.statusTimestamp == "undefined" || (serverTime.getTime() - e.statusTimestamp > 864e5)) {
    ...
    displayScoreDiff(e) {
        ...
        Util.addDom("div", {
            ...
            child: `Last Update ${formatDateTime(t)}`
        }), document.querySelectorAll("#ranks tbody tr").forEach((t => {
            document.querySelector("#"+ t.id).classList.add("ogl_normalScore");
...
class TopbarManager extends Manager {
    ...
    upgradeList() {
        ...
        upgradesToShow.forEach(upgrade => {
            ...
            let a_label = Util.addDom('span', { id: upgrade.planetId+'-'+upgrade.line, child: upgrade.countdown, parent: upgradeListDiv, class: 'tooltip ogl_ready', title: '<div class="htmlTooltip" style="text-align: center">'+formatedDate+'</div>'});
                ...
                const icon = Util.addDom('div', { class:`material-icons ogl_fleetIcon ogl_mission${missionType}`, parent:targetDom, 'data-list':missionsByType[missionType].length, onclick:() =>
                    {
                        ...
                        missionsByType[missionType].forEach(line => {
                            ...
                            const domLine = Util.addDom('div', { class:`ogl_mission${line.mission} ogl_sideFleetIcon`, child:`<div class="material-icons">${line.from.isMoon ? 'bedtime' : 'language'}</div><div>${line.from.name} <small>[${line.from.coords}]</small></div><span>${Util.formatToUnits(shipAmount)}</span><img class="tooltip tooltipRight fleetinfo" src="${fleetImg}" title="<div class='htmlTooltip'>`+a+`</div>"><div class="material-icons">${line.mission == 8 ? 'debris' : line.to.isMoon ? 'bedtime' : 'language'}</div><div>${line.to.name} [${line.to.coords}]</div>`, parent:container });
...
class ShortcutManager extends Manager {
    ...
    getRedirectionLink(e) {
        e = e || {};
        let t = `https://${window.location.host}/game/index.php?page=ingame`;
