                ...
                const icon = Util.addDom('div', { class:`material-icons ogl_fleetIcon ogl_mission${missionType}`, parent:targetDom, 'data-list':missionsByType[missionType].length, onclick:() =>
                    {
                        ...
                        missionsByType[missionType].forEach(line => {
                            ...
                            if (missionType != 18) {
                                Object.keys(line).filter(element => parseInt(element)).forEach(shipID =>{
                                    a+="<div class='ogl_icon ogl_"+shipID+"'><table width='100%' style='margin-left: 5px'><tr><td style='text-align: left'>"+this.ogl._lang.find(shipID)+"</td><td style='text-align: right'>"+Util.formatToUnits(line[shipID]).replace(/"/g, "'")+"</td></tr></table></div>";
                        ...
                        ['metal', 'crystal', 'deut', 'food'].forEach(res => Util.addDom('div', { class:`ogl_icon ogl_${res}`, parent:total, child:Util.formatToUnits(cumul[res] || 0) }));
